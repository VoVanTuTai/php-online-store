<?php
include_once("../class_models/class_control.php");
$x = new control();
...
<div class="container-fluid" style="background-color: white;">  
     <!--Container chứa toàn bộ trang-->
        ...
        <div class="row text-center">
                <div class="col-12">
                <?php
                    include_once('../class_models/class_user.php');
                    ...
                    if (isset($_POST['xoa']) && isset($_POST['idsp'])) {
                        $idsp = (int)$_POST['idsp'];
                        $stmt = $conn->prepare("DELETE FROM giohangnguoidung WHERE idnguoidung = ? AND idsp = ?");
                        $stmt->bind_param("ii", $idNguoiDung, $idsp);
                        $stmt->execute();
                    }

                    // Hiển thị giỏ hàng
                    $user->hienThiGioHang($conn, $idNguoiDung);

                    // Kiểm tra nếu có sản phẩm thì hiển thị nút đặt hàng
                    $kiemtra = $conn->prepare("SELECT COUNT(*) AS total FROM giohangnguoidung WHERE idnguoidung = ?");
                    $kiemtra->bind_param("i", $idNguoiDung);
                    $kiemtra->execute();
                    $kiemtra->bind_result($total);
                    $kiemtra->fetch();
                    $kiemtra->close();
                    ...
                    if ($total > 0) {
                        ...
                        <div class="text-center mt-4">
                            ...
                            <a href="../html/home.php"><button type="button" class="btn btn-success">Trở về trang chủ</button></a>
                        </div>';
                    }
                ?>
                </div>
                <a href="../html/trangthaidonhang.php"><button type="button" class="btn btn-success mt-1">Xem đơn hàng đã đặt</button></a>
        </div>
